...
{
    [Header("Player Data")] [SerializeField, ReadOnly]
    private PlayerLevelScorePair playerLevel;

    [SerializeField, ReadOnly] private int dataCollected;
    [SerializeField, ReadOnly] private int currentData;
    [SerializeField] private int threatLevel;
    [SerializeField, ReadOnly] private int currentThreatLevel;
    [SerializeField] private float levelUpRatio;

    [Header("Controls")] [SerializeField] private float receptorCostMultiplier;
    [SerializeField] private int currentReceptorCost;
    [SerializeField] private float increaseCostChance;
    [SerializeField, ReadOnly] private float dataReceptorRatio;
    [SerializeField] private Vector2 keyControllerAmount;
    ...
    [SerializeField] private ThreatLevelController threatLevelController;
    [SerializeField] private OSController osController;
    [SerializeField] private Material mainGameMaterial;
    [SerializeField] private AudioSource audioPlayer;

    [Header("Databases")] [SerializeField] private PlayerLevelDatabase playerLevelDatabase;
    [SerializeField] private ReceptorController receptorPrefab;
    [SerializeField] private ReceptorKeyInputDatabase keyInputDatabase;

    [Header("Sounds")] [SerializeField] private AudioClip damageSound;
    [SerializeField] private AudioClip bonusSound;
    [SerializeField] private AudioClip collectSound;
    [SerializeField] private AudioClip receptorSound;

    [Header("Colors")] [SerializeField] private Color damageColor;
    ...
    public bool ConvertRandomReceptorToGate()
    {
        ...
        ConvertReceptor(convertedReceptor);
        return true;
    }
    ...
    public bool ConvertReceptorToGateByIndex(int index)
    {
        ...
        ConvertReceptor(currentReceptors[index]);
        return true;
    }

    private void ConvertReceptor(ReceptorController convertedReceptor)
    {
    ...
    public bool MutateRandomReceptor()
    {
        if (currentReceptors.Count <= 0) return false;
        var mutatedReceptor = RandomHelper<ReceptorController>.GetRandomFromList(currentReceptors);
        MutateReceptor(mutatedReceptor);
        return true;
    }

    public bool MutateReceptorByIndex(int index)
    {
        if (currentReceptors.Count <= 0) return false;
        
        index = AssessUtils.MakeValidIndex(ref currentReceptors, index);
        MutateReceptor(currentReceptors[index]);
        return true;
    }

    private void MutateReceptor(ReceptorController receptorController)
    {
        var newCommands = receptorController.Mutate(keyInputDatabase.GetRandomKeyInput());
        impulseSource.GenerateImpulse();
        Console().AddConsoleLine("Mutation.", "$");
        Console().AddConsoleLine(newCommands);
        audioPlayer.PlayOneShot(bonusSound);

        currentThreatLevel = Mathf.Clamp(currentThreatLevel - 1, 0, threatLevel);
        threatLevelController.UpdateThreatLevel((float)currentThreatLevel / threatLevel);
        osController.ReduceIntervalByMargin(0.35f);

        var vec4 = new Vector4(mutationColor.r, mutationColor.g, mutationColor.b, 1.0f);
        AnimateMaterialProperty.AnimateProperty(mainGameMaterial, "_AdditiveColor",
            vec4,
            0.3f, () =>
            {
    ...
    public void ThreatLevelIncrease(int threatDamage)
    {
        currentThreatLevel -= threatDamage;
        threatLevelController.UpdateThreatLevel((float)currentThreatLevel / threatLevel);
        osController.ReduceIntervalByMargin(1.0f * threatLevel);
    }
    ...
    public string GetListOfReceptorCommands()
    {
        ...
        var i = 0;
        currentReceptors.ForEach(receptor => { listOfReceptorCommands += $"RECEPTOR [{i++}] = {receptor}\r\n\r\n"; });
    ...
    private void SpawnReceptor()
    {
        ...
        var receptorController =
            Instantiate(receptorPrefab, new Vector3(position.x, position.y, 0.0f), Quaternion.identity);
