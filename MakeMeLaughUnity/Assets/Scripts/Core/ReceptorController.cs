...
namespace Core
{
    public class ReceptorController : AgnosticCollisionSolver
    {
        ...
        public string Mutate(KeyCodePair newKeyCodePair)
        {
            var direction = RandomPointUtils.GenerateRandomDirection2Din3D();
            commandKeys.Add(new KeyCodeDirectionPair(newKeyCodePair.One, direction));
            commandKeys.Add(new KeyCodeDirectionPair(newKeyCodePair.Two, -direction));
            var currentScale = transform.localScale;
            transform.DOScale(currentScale + new Vector3(0.5f, 0.5f, 0.5f), 0.25f);
